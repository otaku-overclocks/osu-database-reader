...
namespace osu_database_reader
{
    public class CustomReader : BinaryReader
    {
        ...
        {
            // an overload to ReadBytes(int count)
        ...
        public override string ReadString()
        {
            ...
            if (b == 0x0B)
            {
                var ret = base.ReadString();
                //Debug.WriteLine("Filled" + ret);
                return ret;
            }
            else if (b == 0x00)
            {
                ...
                throw
                    new Exception(
                        $"Continuation byte is not 0x00 or 0x11, but is 0x{b.ToString("X2")} (position: {BaseStream.Position})");
        }
